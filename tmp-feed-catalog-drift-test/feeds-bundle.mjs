...
function resolveRegisteredTelegramSourceName(handle) {
...
function resolveTelegramSourceName(channelTitle, handle) {
  const trimmedHandle = handle?.trim();
...
function getSourceTierBadgeTitle(sourceType) {
...
function describePropagandaBadge(profile, sourceType = "unknown") {
  ...
  const title = profile.note || (sourceType === "gov" ? `Official government source${profile.stateAffiliated ? `: ${profile.stateAffiliated}` : ""}` : profile.stateAffiliated ? `State-affiliated: ${profile.stateAffiliated}` : "Provenance not yet reviewed");
  if (sourceType === "gov") {
    return {
      risk: profile.risk,
...
var _sourcePanelMap = null;
function getSourcePanelId(sourceName) {
  if (!_sourcePanelMap) {
    _sourcePanelMap = /* @__PURE__ */ new Map();
...
function listConfiguredFeedNames() {
...
function getFeedProvenanceState(sourceName) {
...
function getLocaleBoostedSources(locale) {
  return getLanguageMatchedSources(locale);
}
function computeDefaultDisabledSources(locale) {
...
function computeLegacyDefaultDisabledSources() {
