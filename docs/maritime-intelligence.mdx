...
The system monitors the canonical strategic-waterway registry for disruptions that could affect global trade. It uses IMF PortWatch for weekly vessel transit counts, AISStream for the real-time crossing counter, and CorridorRisk for risk intelligence.
...
| IMD port / sea / coastal | Bootstrap `imdCycloneMarine` (`weather:imd-cyclone-marine:v1`) | Official IMD port warnings and sea-area / coastal bulletins with issuing ACWC/CWC, valid times, wind, visibility, and sea state. Not merged into NWS weather alerts. Fishermen warning stays disabled (indexed, no field reference). |
